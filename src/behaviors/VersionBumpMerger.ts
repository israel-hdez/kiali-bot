...
interface PullRequestToValidate {
  draft: boolean;
  merged: boolean;
  number: number;
  title: string;
...
export default class VersionBumpMerger extends Behavior {
  ...
  public constructor(app: Application) {
    super(app);
    this.installPrCreatedHandler(app);
    this.installCheckRunCompletedHandler(app);
    this.installCommitStatusChangedHandler(app);
  ...
  private installCheckRunCompletedHandler(app: Application): void {
    app.on(
      'check_run.completed',
  ...
  private installCommitStatusChangedHandler(app: Application): void {
    app.on(
      'status',
      async (context): Promise<void> => {
  ...
  private installPrCreatedHandler(app: Application): void {
    app.on(
      'pull_request.opened',
      async (context): Promise<void> => {
  ...
  private static isValidPr = (pr: PullRequestToValidate): string | null => {
